...
| `stamp context style [path] [options]` | Generates context with style metadata included. Extracts visual and layout information (Tailwind, SCSS, animations, layout patterns). Equivalent to `stamp context --include-style`. | Design system analysis, AI-assisted design suggestions, layout understanding, animation detection. | All `stamp context` options supported. |
...
- Run `stamp context style` to generate context with style metadata (Tailwind, SCSS, animations, layout patterns). Equivalent to `stamp context --include-style`.
